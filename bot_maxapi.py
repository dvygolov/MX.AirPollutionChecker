...
class StateStore:
    """SQLite: подписчики, last_alert_id, last_snapshot."""
    ...
    def was_alert_sent(self, alert_hash: str) -> bool:
        with self.lock:
            row = self.conn.execute(
                "SELECT 1 FROM alert_history WHERE alert_hash = ?", (alert_hash,)
            ).fetchone()
...
class AirDataClient:
    ...
    async def fetch_latest(self, station: int = 11) -> dict[str, Any]:
        ...
        response = await self.http.get(POLL_API_URL, params=params)
    ...
    async def fetch_comment(self, station: int = 11) -> str:
        params = {"station": station}
        response = await self.http.get(COMMENT_API_URL, params=params)
        response.raise_for_status()
        data = response.json()
        station_data = data.get(str(station), {})
        comment = station_data.get("comment_last", {})
...
class AirPollutionBot:
    STATE_KEY_LAST_BEGIN_AT = "last_begin_at"
    ...
    async def handle_status(self) -> str:
        try:
            ...
            for _, meas_id_str, meta, last in poll_items:
                ...
                line = (
                    f"{emoji} <b>{name}</b>{warning}\n"
                    f"└ <code>{value} / {limit} мг/м³</code> — <b>{pct:.1f}%</b> ПДК"
                )
                lines.append(line)

            meteo_parts: list[str] = []
            for meas_id_str, meta in meas_list.items():
    ...
    async def handle_help(self) -> str:
        return (
            "<b>🛡 Команды бота</b>\n\n"
            "<code>/start</code> — подписаться на уведомления о превышении ПДК\n"
            "<code>/stop</code> — отписаться\n"
            "<code>/status</code> — текущее качество воздуха с индикаторами\n"
            "<code>/help</code> — эта справка\n\n"
            "<b>О мониторинге</b>\n"
            "Данные поступают с автоматического поста "
            "№91 (Софийская площадь, р-н Волгарь) "
            "от ФГБУ «Приволжское УГМС». "
            f"Бот проверяет значения каждые "
            f"{self.settings.poll_interval_sec // 60} минут и сразу же шлёт алерт "
            f"при превышении порога <b>{self.settings.factor_threshold}×ПДК</b>."
        )
    ...
    async def check_and_alert(self, bot: Bot) -> list[dict[str, Any]]:
        ...
        last_begin_at = self.store.get_state(self.STATE_KEY_LAST_BEGIN_AT)

        if current_begin_at == last_begin_at:
            return []

        if current_begin_at:
            self.store.set_state(self.STATE_KEY_LAST_BEGIN_AT, current_begin_at)
        ...
        for meas_id_str, meta in meas_list.items():
            ...
            if factor > self.settings.factor_threshold:
    ...
    async def _send_alert(
        ...
        text = (
            ...
            f"<i>Срок: {begin_at}</i>\n"
            f"<i>Пост №91, Софийская площадь, р-н Волгарь</i>"
        )
        ...
        self.store.record_alert(alert_hash, meas_id, factor, value, name)
        logging.warning(
            "ALERT sent: %s (factor=%.2f) to %d/%d subscribers",
            name, factor, sent_count, len(subscribers),
        )
        return {
            "meas_id": meas_id,
            "fullname": name,
    ...
    while True:
        try:
            ...
            if alerts:
                for a in alerts:
                    logging.info(
                        "Alert: %s (%.2f×ПДК) reached %d/%d users",
                        a["fullname"],
                        a["factor"],
                        a["subscribers_reached"],
                        a["subscribers_total"],
                    )
        except Exception:
...
async def main() -> None:
    ...
    async def on_start(event: MessageCreated) -> None:
        reply = await app.handle_start(event.message.chat.id)
    ...
    async def on_stop(event: MessageCreated) -> None:
        reply = await app.handle_stop(event.message.chat.id)
        await event.message.answer(text=reply, format="html")
